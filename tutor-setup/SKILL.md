...
description: >
  ...
  Triggers on "tutor-setup", "create study vault", "build study notes", "create onboarding vault", "StudyVault".
...
2. **Extract text (MANDATORY tools)**:
   - **PDF → `pdftotext` CLI ONLY** (run via Bash tool). NEVER use the Read tool directly on PDF files — it renders pages as images and wastes 10-50x more tokens. Convert to `.txt` first, then Read the `.txt` file.
     ...
     ```
   - If `pdftotext` is not installed, install it:
     - **macOS**: `brew install poppler`
     - **Linux**: `apt-get install poppler-utils`
     - **Windows**: `choco install poppler` or `scoop install poppler` (adds `pdftotext` to PATH). If neither package manager is available, download poppler from https://github.com/oschwartz10612/poppler-windows/releases and add the `bin/` folder to PATH.
   - URL → WebFetch
